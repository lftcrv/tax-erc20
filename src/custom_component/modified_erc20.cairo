...
pub mod ERC20Component {
    ...
    use openzeppelin::token::erc20::snip12_utils::permit::Permit;
    use openzeppelin::account::interface::{ISRC6Dispatcher, ISRC6DispatcherTrait};
    use openzeppelin::utils::cryptography::interface::{INonces, ISNIP12Metadata};
    ...
    pub struct Storage {
        ...
        pub ERC20_is_pool: Map<ContractAddress, bool>,
        pub ERC20_sell_tax: u16,
        pub ERC20_buy_tax: u16,
        pub ERC20_tax_contract: ContractAddress,
    }
    ...
    > of InternalTrait<TContractState> {
        ...
        fn _transfer(
            ...
            let is_sender_pool = self.ERC20_is_pool.read(sender);
            let is_recipient_pool = self.ERC20_is_pool.read(recipient);
            if is_sender_pool == is_recipient_pool {
                return self.update(sender, recipient, amount);
            };
            if is_recipient_pool {
                let tax = self.ERC20_sell_tax.read();
                let tax_amount = amount * tax.into() / 10_000;
                let amount_after_tax = amount; // - tax_amount;
                if tax_amount > 0 {
                    // self.update(sender, self.ERC20_tax_contract.read(), tax_amount);
                    self.mint(self.ERC20_tax_contract.read(), tax_amount);
                }
                self.update(sender, recipient, amount_after_tax);
            } else {
                panic!("is_sender_pool  -- buying");
        ...
        /// TAX related
        fn set_sell_tax(ref self: ComponentState<TContractState>, sell_tax: u16) {
            self.ERC20_sell_tax.write(sell_tax);
        }
        fn set_buy_tax(ref self: ComponentState<TContractState>, sell_tax: u16) {
            self.ERC20_buy_tax.write(sell_tax);
        }
        fn set_is_pool(ref self: ComponentState<TContractState>, address: ContractAddress) {
            self.ERC20_is_pool.write(address, true);
        }
        fn revoke_is_pool(ref self: ComponentState<TContractState>, address: ContractAddress) {
            self.ERC20_is_pool.write(address, false);
        }
        fn set_tax_address(ref self: ComponentState<TContractState>, address: ContractAddress) {
            self.ERC20_tax_contract.write(address);
        }
    }
}
